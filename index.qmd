---
title: WRAIR AFRICA DASHBOARD | SEPTEMBER 2024
...
prog_data <- read_csv("data/prog_data_sep24.csv")

prog_targets <- read_csv("data/fy24_targets.csv")

vl_data <- read_csv("data/vl_data_sep24.csv")

prog_site <- read_csv("data/prog_sites.csv")

vl_casc <- read_csv("data/vl_data_agesex_sep24.csv")
...
data_fil <- prog_data_site %>%
  filter(period >= ymd("2023-10-01") & period <= ymd("2024-09-30"))
...
vl_casc_aug <- vl_casc %>%
  filter(period == ymd("2024-09-01") & age_group != "All") %>% 
  filter(data !="TX_NEW")
...
vl_casc1 <- vl_casc %>%
  filter(data == "TX_NEW" & month %in% c("Apr", "May", "Jun", "Jul","Aug", "Sep") & year == 2024) %>% 
...
tx_new_mar_aug<- data_fil %>% 
  filter(indicator == "TX_NEW_R",year == 2024 & month %in% c("Apr", "May", "Jun", "Jul","Aug", "Sep")) %>% 
...
cut_off_dt <- as.Date("2024-09-30", format="%Y-%m-%d")
