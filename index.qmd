---
title: WRAIR AFRICA DASHBOARD | OCTOBER 2024
...
prog_data <- read_csv("data/prog_data_oct24.csv")

prog_targets <- read_csv("data/fy25_targets.csv")

vl_data <- read_csv("data/vl_data_oct24.csv")

prog_site <- read_csv("data/prog_sites.csv")

vl_casc <- read_csv("data/vl_data_agesex_oct24.csv")

biom_update <- read_csv("data/biometric_update.csv")

radet <- read_csv("data/radet_oct24.csv")
...
data_fil <- prog_data_site %>%
  filter(period >= ymd("2024-10-01") & period <= ymd("2025-09-30"))


data_fil_cum <- prog_data_site %>% 
  filter(indicator == "TX_CURR_R" | indicator == "TX_PVLS_D_R" | indicator == "OVC_SERV_CP_R",
         month == "Oct")
...
data_sum <- data_sum %>% 
  filter(!indicator %in% c("TX_CURR_R", "TX_PVLS_D_R", "TX_PVLS_N_R", "TX_TB_R", "OVC_SERV_CP_R"))
...
# Add OVC_SERV_CP_R and OVC_SERV_PR_R to create new indicator OVC_SERV, drop existing indicators, and change prog_area to OVC_SERV

data_sum_r <- data_sum_r %>%
  filter(indicator %in% c("OVC_SERV_CP_R", "OVC_SERV_PR_R")) %>%  # Filter the relevant indicators
  summarise(
    indicator = "OVC_SERV_R",           # Create the new indicator
    total_value = sum(total_value),     # Sum the total_value
    prog_area = "OVC_SERV"              # Set prog_area to "OVC_SERV"
  ) %>%
  bind_rows(
    data_sum_r %>%                      # Add back the other rows
      filter(!indicator %in% c("OVC_SERV_CP_R", "OVC_SERV_PR_R"))
  )
...
prog_targets_t <- clean_names(prog_targets) %>% 
  mutate(prog_area = sub(" Target$", "", target_code))
...
vl_casc_aug <- vl_casc %>%
  filter(period == ymd("2024-10-01") & age_group != "All") %>% 
  filter(data !="TX_NEW")
...
vl_casc1 <- vl_casc %>%
  filter(data == "TX_NEW" & month %in% c("May", "Jun", "Jul","Aug", "Sep", "Oct") & year == 2024) %>% 
...
```{r}
# Sum of TX_NEW from Apr 2024 to Sep 2024 to determine eligibility for VL testing

tx_new_oct<- data_fil %>% 
  filter(indicator == "TX_NEW_R",year == 2024 & month %in% c("May", "Jun", "Jul","Aug", "Sep", "Oct")) %>% 
...
# Filter the data for October 2024

vl_data_jul <- vl_data_site %>%
  filter(period == ymd("2024-10-01"))
...
eligible_vl <- tx_curr - tx_new_oct
...
cut_off_dt <- as.Date("2024-10-31", format="%Y-%m-%d")
...
#radet <- radet %>% 
 # mutate(facility = ifelse(facility =="ba Nigerian Army Amoured Centre Medical Reception Station Bauchi", "ba 33 Brigade Medical Center Shadawanka", facility)) %>%
  ...
  mutate(patient_status = 
        ...
        facility_name =
          case_when(
            facility_name == "82DMSH Enugu" ~ "82 Division Medical Services and Hospital",
            TRUE ~ facility_name
          )
  )
    ...
    mutate(
      ...
      regimen_line = 
           case_when(regimen_line == "Adult 1st Line" ~ "1st Line",
                     ...
                     regimen_line == "Pediatric Third Line" ~ "3rd Line",
                     TRUE ~ regimen_line),
...
# Create a dataset from the RADET for VL coverage (had a vl / eligible for vl) by sex and age group

radet_vl_cov <- radet %>%
  filter(eligible_vl == "Eligible") %>%
  group_by(sex, age_group) %>%
  summarise(
    current_vl = sum(current_vl == "Current VL", na.rm = TRUE),
    total_eligible = n()
  ) %>%
  mutate(vl_cov = current_vl / total_eligible)
...
```{r}
# rename systolic and diastolic blood pressure fields

radet <- radet %>% 
  rename(systolic_bp = systolic_blood_pressure_mm_hg,
         diastolic_bp = diastolic_blood_pressure_mm_hg)

```


```{r}
# Determine the blood pressure status for every patient
  ...
  mutate(
    bp_status = 
      case_when(
        ...
        systolic_bp < 120 & diastolic_bp < 80 ~ "Optimal",
        systolic_bp < 130 & diastolic_bp < 85 ~ "Normal",
        (systolic_bp >= 130 & systolic_bp <= 139) | (diastolic_bp >= 85 & diastolic_bp <= 89) ~ "High Normal",
        (systolic_bp >= 140 & systolic_bp <= 159) | (diastolic_bp >= 90 & diastolic_bp <= 99) ~ "Grade 1 Hypertension",
        (systolic_bp >= 160 & systolic_bp <= 179) | (diastolic_bp >= 100 & diastolic_bp <= 109) ~ "Grade 2 Hypertension",
        systolic_bp >= 180 | diastolic_bp >= 110 ~ "Grade 3 Hypertension",
        systolic_bp >= 140 & diastolic_bp < 90 ~ "Isolated Systolic Hypertension",
    TRUE ~ "Unknown"
      )
  )
...
```{r}
# Write radet dataset to CSV

# write_csv(radet, "radet_oct.csv")
...
# radet_bp <- radet %>%
  # select(facility_name, patient_id, sex, age, age_group, status_check, systolic_bp, diastolic_bp, bp_status)
...
```{r}
# bp_summary <- radet_bp %>%
  # filter only active patients from status_check and filter out NA values from bp_status2
 # filter(status_check == "Active") %>%
  # filter(!is.na(bp_status))
...
# bp_sum <- bp_summary %>% 
 ...
 # summarise(
  #  count = n(),
  #  percentage = (n() / nrow(bp_summary)) * 100
 # )

# Display the data in descending order of percentage
...
# Plot vl_cov by sex and age group

vl_cov_plot <- radet_vl_cov %>%
  ggplot(aes(x = age_group, y = vl_cov, fill =sex)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = scales::percent(vl_cov, accuracy = 1)), vjust = -0.5, size = 4) +
  labs(
    title = "Viral Load Coverage by Age Group",
    x = "Age Group",
    y = "Coverage (%)"
  ) +
  #scale_fill(discrete = TRUE) +
  theme_minimal() +
  theme(
...
```{r}
value_box(
  title = tags$span("Current on Treatment: Prelim", style = "font-size: 30px;font-weight:bold;"),
...
value_box(
  ...
  theme = value_box_theme(bg ="#4b5539")
)
```
...
value_box(
  ...
  theme = value_box_theme(bg ="#6c7735")
)
```
...
value_box(
  ...
  theme = value_box_theme(bg ="#22bb11")
)
```
...
```{r echo=FALSE, results='asis', warning=FALSE, message=FALSE, title="PROGRAM AREA PERFORMANCE AGAINST TARGETS | OCTOBER 2023 - SEPTEMBER 2024", height=550, width = 800}
